...
settings=("accept_threshold_percent" "compressor" "enabled" "max_pool_percent" "shrinker_enabled")
# Array of zswap statistics
statistics=("pool_limit_hit" "pool_total_size" "reject_alloc_fail" "reject_compress_fail" "reject_compress_poor" "reject_kmemcache_fail" "reject_reclaim_fail" "stored_pages" "stored_incompressible_pages" "decompress_fail" "written_back_pages")
...
if [ "${zswap_data["stored_pages"]}" -ne 0 ] && [ "${zswap_data["pool_total_size"]}" -ne 0 ]; then
    compression_ratio=$(awk "BEGIN { printf \"%.2f\", (${zswap_data["stored_pages"]} * $page_size) / ${zswap_data["pool_total_size"]} }")
else
    compression_ratio=0
fi
